...
export default function PaymentPage() {
  ...
  useEffect(() => {
    ...
    s.onerror = () =>
      toast.error("Could not load Paystack. Check your internet.");
  ...
  const onSuccess = async (orderId, reference) => {
    try {
      await apiClient.createOrder({
        ...
        shippingAddress: {
          ...
          // IMPORTANT
          street: checkoutData.street || checkoutData.address,
      ...
      console.error(err);
      toast.error(err.message || "Unable to save order.");
    }
  };
  const handlePaystack = () => {
    if (!paystackReady) {
      toast.error("Paystack not ready yet. Please wait.");
      return;
    }
    ...
    const handler = window.PaystackPop.setup({
      ...
      callback: async (res) => {
        await onSuccess(orderId, res.reference);
      },
  ...
  const handleDemo = async () => {
    try {
      ...
      await apiClient.demoPayment(order.id);
      ...
      <div className="flex items-center gap-2 mb-10 flex-wrap">
        {["Cart", "Checkout", "Payment", "Done"].map((step, i) => (
          <div key={step} className="flex items-center gap-2">
            ...
            <span
              className={`text-sm ${i === 2 ? "text-white font-medium" : i < 2 ? "text-brand-400" : "text-slate-600"}`}
            >
              {step}
            </span>
        ...
        <div className="space-y-5">
          ...
          <div className="flex items-start gap-3 px-2">
            ...
            <p className="text-xs text-slate-500">
              All payments are secured with 256-bit SSL encryption. Your card
            ...
            <div className="border-t border-white/10 pt-4 space-y-2">
              {[
                ...
                <div key={l} className="flex justify-between text-sm">
                  ...
                  <span
                    className={v === "FREE" ? "text-brand-400" : "text-white"}
                  >
                    {v}
                  </span>
                </div>
